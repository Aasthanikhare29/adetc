...
// Build a query string (?category=&tag=&q=) from active filters (omits empties).
export function blogQuery({ category, tag, q } = {}) {
...
function applyFilters(query, { category, tag, q } = {}) {
  let q2 = query.eq('status', 'published');
...
// Published posts matching the optional filters, page-sliced (page starts at 1).
export async function getFilteredPosts({ category, tag, q } = {}, page = 1) {
  const client = db();
  if (!client) return [];
  const start = (page - 1) * POSTS_PER_PAGE;
  const { data, error } = await applyFilters(client.from('posts').select(SELECT), { category, tag, q })
    .order('published_at', { ascending: false })
    .range(start, start + POSTS_PER_PAGE - 1);
  if (error) { console.warn('[blog] getFilteredPosts:', error.message); return []; }
  return (data || []).map(rowToPost);
}

export async function getFilteredTotalPages({ category, tag, q } = {}) {
  const client = db();
  if (!client) return 1;
  ...
  if (error) { console.warn('[blog] getFilteredTotalPages:', error.message); return 1; }
  return Math.max(1, Math.ceil((count || 0) / POSTS_PER_PAGE));
}
